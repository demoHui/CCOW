...
//#ifdef SESC_COW
#ifdef SESC_COW_UPDATE
bool  OSSim::eventCommit(ThreadContext *pthread)
{
	if(verCmp(pthread)){
		GProcessor* cpu = pid2GProcessor(pthread->getPid());
		cpu->elm.elmCommit(pthread);
		cpu->elm.init();
		return true;
     	}
	else {
		printf("thread %d version error\n",pthread->getPid());
		return false;
	}
}
bool OSSim::verCmp(ThreadContext* pthread)
{
	ThreadContext* main  = ThreadContext::getMainThreadContext();
	for(VTable :: iterator it = pthread->vt.begin();it != pthread->vt.end();it++){
		VersionTag* vTag = main->findVTEntry((*it)->addr,(*it)->size);
		if(!vTag){
			printf("verCmp: main version error:   addr = %x,version = %d\n",(*it)->addr,(*it)->version);
			exit(0);
		}	
		if((*it)->version != main->getDataVersion(vTag))
		{
			printf("data  addr = %x\n",(*it)->addr);
			printf("verCmp : thread %d version = %d, main version = %d\n",pthread->getPid(),(*it)->version,main->getDataVersion(vTag));
			printf("verCmp: error\n");
//			exit(0);
			return false;
		}
...
bool OSSim::isOrderCommit(Pid_t pid)
{
	ThreadContext* thread = allThread.front();
	if(pid == thread->getPid())
		return true;
	else return false;
}

void OSSim::elmClear(Pid_t pid)
{
	GProcessor* cpu = pid2GProcessor(pid);
	cpu->elm.init();
}
void OSSim::eventNonCommitExit()
{
	//copyToReplayQ();
	while(!allThread.empty()){
		replayThread.push(allThread.front());
		ThreadContext* thread = allThread.front();
		getContext(thread->getPid())->clear();
		ThreadContext::getContext(thread->getPid())->clear();
		GProcessor* cpu = pid2GProcessor(thread->getPid());
		if(cpu){
			cpu->elm.init();
		}
		eventFakeExit(thread->getPid(),thread->getIntArg1());
		numReplay ++ ;
	}
}
void OSSim::eventThreadsReplay()
{
	while(!replayThread.empty()){
		ThreadContext* thread = replayThread.front();
		replayThread.pop();
		eventReplay(thread);
	}
}

void OSSim::eventReplay(ThreadContext * child)
{
	child->setPid2Context(child);
	ThreadContext* mainThread = osSim->getContext(0);
	struct ThreadPara prama = child->threadPara;
	if (Maxpid < child->getPid())
    		Maxpid = child->getPid();
	child->shareAddrSpace(mainThread, PR_SADDR, !prama.entry);
       	child->setSpecThread(true);
  	child->init();
  	mainThread->newChild(child);
	
	child->setPCIcode(addr2icode(prama.entry));
	child->setIntReg(IntArg1Reg,prama.intArg1Reg);
	child->setIntReg(IntArg2Reg,prama.intArg2Reg);            /* for sprocsp() */
	child->setIntReg(JmpPtrReg,prama.jmpPtrReg);
	child->setIntReg(RetAddrReg,prama.retAddrReg);
	//mainThread->setRetVal(child->getPid());
	child->clear();
	child->setRetVal(0);
	allThread.push(child);
	eventSpawn(prama.ppid,child->getPid(),0);
}
void OSSim::copyToReplayQ()
{
	while(!allThread.empty()){
		replayThread.push(allThread.front());
		allThread.pop();
...
void OSSim::threadsExit()
{
	DeadThreads::iterator it = dts.begin();
	for(;it!=dts.end();it++){
//		printf("INFO: pid %d exit\n",(*it)->pid);
		eventExit((*it)->pid,(*it)->err);
		if((*it)->specThread)
			osSim->threadnum++;
	}
	dts.clear();
...
//  #ifdef SESC_COW_REPLAY
  ...
  allThread.pop();
#endif
  ThreadContext::getContext(cpid)->freePid();
}
...
void OSSim::initBoot()
{
  ...
  if (SescConf->checkBool("","NoMigration"))//bind thread  with processor
