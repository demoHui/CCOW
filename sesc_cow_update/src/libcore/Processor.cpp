...
void Processor::advanceClock()
{
  ...
  // Fetch Stage
 // printf("INFO: advance clock has work:%d at time %ld\n",IFID.hasWork(),globalClock);
  if (IFID.hasWork() ) {
...
/*  if(spaceInInstQueue<FetchWidth&&getThreadContext(0)->getPid())
  	printf("INFO: id stage pid %d, no space for id stage(%d,%d)\n",getThreadContext(0)->getPid(),spaceInInstQueue,FetchWidth);
*/  if (spaceInInstQueue >= FetchWidth) {
  ...
  // RENAME Stage
/*
  if(getThreadContext(0)->getPid()&&pipeQ.instQueue.empty())
  	printf("INFO: pid %d, inst queue empty, space in inst queue %d\n",getThreadContext(0)->getPid(),spaceInInstQueue);
...
/*allocate ROB entry  dest register  resource & cluster to execute*/
  StallCause sc = sharedAddInst(dinst);
  if (sc != NoStall)
  {
//  	printf("INFO: error %d issue\n",sc);
    return sc;
  }
  
#ifdef SESC_COW_DEBUG
 if(!dinst->getResource()){printf("addInst : dinst = %x\n",dinst);exit(0);}
#endif
