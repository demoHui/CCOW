...
StallCause GProcessor::sharedAddInst(DInst *dinst) 
{
 ...
 Resource *res = clusterManager.getResource(inst->getOpcode());
  ...
  if(!res)	{printf("GProcessor : sharedAddrInst  dinst = %x\n",dinst);exit(0);}
  #endif
...
//#if(SESC_COW_TIME_PENDING)
    ...
    if(dinst->isExitInst()&&dinst->isSpecInst()){
  	Resource *res = clusterManager.getResource(iEvent);//SET RESOURCE TO BE IEVENT IS BAD, WE NEED TO KNOW MINT_EXIT IS EVENT OR INST
	dinst->setResource( res);
//	printf("INFO: gp retire : cpuid %d exit inst retire\n",Id);
    }
#endif
