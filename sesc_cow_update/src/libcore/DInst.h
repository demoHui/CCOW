...
private:
  ...
  DInstNext pend[MAX_PENDING_SOURCES];//op sources
  DInstNext *last;
  DInstNext *first;
...
*/
 ...
 #if( SESC_COW_TIME_PENDING||SESC_COW_STLS)
  void commitDinst();
  StaticCallbackMember0<DInst,&DInst::commitDinst> commitDinstCB;	   
#endif
*/
  DInst *clone();
...
///BEGIN Version
  ...
  void incDataVersion(){dataVersion++;}

  void setRemotePid(Pid_t pid){remotePid = pid;}
  Pid_t getRemotePid(){return remotePid;}
  void setReqVersion(int version){reqVersion = version;}
  int getReqVersion(){return reqVersion;}

  void setSpecInst(bool spec){specInst = spec;}
  bool isSpecInst(){return specInst;}

  void setExecPath(ExePath path) {exePath = path;}
  ExePath getExecPath(){return exePath;}

  void setExitInst(bool exit){exitInst = exit;}
  bool isExitInst(){return exitInst;}
   void setRaddr(RAddr addr){raddr = addr;}
   RAddr getRaddr(){return raddr;}
