...
//#ifdef SESC_COW_REPLAY
#ifdef SESC_COW_UPDATE
  typedef std::queue<ThreadContext *> RunningThread;
  RunningThread allThread;
  RunningThread replayThread;
  int numReplay;
  ...
  typedef std::vector<ExitThread*> DeadThreads;
  DeadThreads dts;
  int threadnum;
  int recordThread;
#endif
...
//#ifdef SESC_COW_VERSION
  ...
  void eventThreadsReplay();
  void eventReplay(ThreadContext * thread);
  void copyToReplayQ();
