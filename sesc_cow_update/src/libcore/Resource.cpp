...
/*i think it is not neccessary, so we don't use it in the update*/

#ifdef SESC_COW_DEBUG
	printf("INFO: CacheDispatched dinst = %x\n",dinst);
#endif
...
/*
#ifdef SESC_COW
void FULoad::elmDispatched(DInst *dinst)
{
//	fprintf(stderr,"FULoad EMemRequest\n");
	EMemRequest::create(dinst, memorySystem, MemRead);
}
#endif 
*/
...
#ifdef SESC_COW_UPDATE
  if(dinst->getExecPath() == ECACHE)
  	{
  		SMemRequest::create(dinst,memorySystem,MemSpecRead);
		return;
  	}
  if(dinst->getExecPath() == REMOTEACCESS)
  	{
 // 		printf("INFO: CacheDispatched REMOTEACCESS create\n");
  		SMemRequest::create(dinst,memorySystem,MemSpecReadRemote);
		return;
  	}
#endif
  ...
  #ifdef SESC_COW_DEBUG
    printf("INFO: DMemRequest dinst = %x\n",dinst);
  #endif
...
StallCause FUStore::canIssue(DInst *dinst)
{
  ...
  /*to enforce ld st dependencies*/
  LDSTBuffer::getStoreEntry(dinst);
  cluster->getGProcessor()->getLSQ()->insert(dinst);

  if (dinst->isFake()) {//always false for no mispath
    misStores++;
  }else{
    freeStores--;
  }
...
{
/*for these no use inst, we just let it fly*/
if (dinst->isDeadInst() || dinst->isFake() || dinst->isEarlyRecycled()
  ...
  if(dinst->getExecPath() == ECACHE)
  	{
  		if(ecache->getNextFreeCycle() > globalClock)
			return NoCachePorts;
		gen->nextSlot();

	/*i don't know what it is for, so just print some imformation */
		if(!dinst)
			{
//				printf("INFO : FUStore dinst missing\n");
				exit(0);
			}
		if(dinst->getPendEvent())
			...
			{
				dinst->getPendEvent()->call();
//				printf("INFO: PendEvent is executed, but the event is a system call.\n");
				exit(0);
			}
		if(waitingOnFence() == true)
			//return WaitForFence;
			//what is wait for fence
			{
//				printf("INFO: Wait For Fence! Oh , NO~\n");
				exit(0);
			}
		else
			storeSent();
		SMemRequest::create( dinst, memorySystem, MemSpecWrite);
		doRetire(dinst);
		
		return Retired;
		
  	}//end if  dinst->exePath == ECACHE
#endif
...
/***********************************************/
#ifdef SESC_COW_UPDATE
FUEvent::  FUEvent(Cluster * cls, GMemorySystem *gmem)
:Resource(cls,0)
,memorySystem(gmem)
{
}
...
/*for exit inst, i think we can do the memspec commit here*/
...
#ifdef SESC_COW_UPDATE
RetOutcome FUEvent::retire(DInst * dinst)
{
  	if(dinst->isExitInst() && dinst->isSpecInst())
  	{
//  		printf("exit = %d, spec = %d\n",dinst->isExitInst(),dinst->isSpecInst());
//  		printf("INFO: thread exit is executed here, do something\n");
		//exit(0);
		SMemRequest::create(dinst,memorySystem,MemSpecCommit);
  	}
	return Retired;
}
#endif
