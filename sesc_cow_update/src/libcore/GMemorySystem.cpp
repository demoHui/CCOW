...
MemObj *GMemorySystem::buildMemoryObj(const char *type, const char *section, const char *name)
{
  if (!(strcasecmp(type, "dummy") == 0
	...
	|| strcasecmp(type,"elm") == 0
	|| strcasecmp(type,"nicecache") == 0
	|| strcasecmp(type,"systembus") == 0
	)) {
    MSG("Invalid memory type [%s]", type);
  }
#ifdef MEMLEVEL
	  printf("GMemorySystem:buildMemoryObj  section = %s,name = %s\n",section,name);
#endif
...
void GMemorySystem::buildMemorySystem()
{
  ...
  dataSource = declareMemoryObj(def_block, "dataSource");//memorysystem::declareMemoryObj
  if (dataSource) {
...
//#ifdef SESC_COW
#ifdef SESC_COW_UPDATE
  specSource = declareMemoryObj(def_block,"specSource");
  if(specSource){
    specSource->setHighestLevel();
    specSource->computenUpperCaches();
  }
#endif
  memoryOS = buildMemoryOS(def_block);//memorysystem::buildMemoryOS
...
//#ifdef SESC_COW
#ifdef SESC_COW_UPDATE
MemObj *GMemorySystem::getSpecSource() const
{
	return specSource;
}
#endif 
