...
void DInst::doAtSimTime()
{
  ...
  if (!isStallOnLoad())//why it always being true??
...
void DInst::doAtSelect()
{
  I(resource->getCluster());
   ...
   if(!getResource()){printf("do at select  : dinst = %x\n",this);exit(0);}
  #endif
...
/*
#if( SESC_COW_TIME_PENDING||SESC_COW_STLS)
void DInst::commitDinst()
{
   #ifndef SESC_COW_STLS
	destroy();
   #endif
   // FUMemory* r = (FUMemory*) c->getResource(iFence);
	resource->commitCompleted();
#ifdef SESC_COW_STLS
	compareFinished = 1;
#endif
...
#endif // Else of (defined MIPS_EMUL)
{
  ...
  DInst *i = dInstPool.out();//allocate the pool space  guohui
