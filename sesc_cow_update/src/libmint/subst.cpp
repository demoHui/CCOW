...
/* {"exit",                    mint_exit,                       1, OpClass(OpUndoable,OpAtStart)}, */
  ...
  /*
  #ifdef SESC_REMOTE
  {"sesc_remote",		mint_sesc_remote,		1,		OpNoReplay},
  #endif
  */
//  #ifdef SESC_COW_REMOTE
#ifdef SESC_COW_UPDATE
   {"sesc_version_remote",		mint_sesc_version_remote,		1,		OpNoReplay},
   {"sesc_atomic",	mint_sesc_atomic,	1,	OpNoReplay},
   {"sesc_main_clear", mint_sesc_main_clear, 1, OpNoReplay},
{"sesc_get_globalClock",mint_sesc_get_globalClock,1, OpNoReplay},
{"sesc_get_num_threads", mint_sesc_get_num_threads, 1, OpNoReplay},
{"sesc_set_num_threads", mint_sesc_set_num_threads, 1, OpNoReplay},
{"sesc_inc_threads", mint_sesc_inc_threads, 1, OpNoReplay},
{"sesc_dec_threads", mint_sesc_dec_threads, 1, OpNoReplay},
 #endif
...
OP(mint_sesc_remote){
   ...
   for(it = pthread->getMainThreadContext()->varVector.begin();it != pthread->getMainThreadContext()->varVector.end();it++){
   		if(it->addr == addr){
				it->threadVector.push_back(pthread);
				return pthread->getRetIcode();
   			}
   	}
   struct variable var ;
   var.addr = addr;
   var.threadVector.push_back(pthread);
   pthread->getMainThreadContext()->varVector.push_back(var);
   return pthread->getRetIcode();
}
#endif
*/
//#ifdef SESC_COW_REMOTE
#ifdef SESC_COW_UPDATE
OP(mint_sesc_get_globalClock)
{
	MintFuncArgs funcArgs(pthread,picode);
	int32_t addr = (RAddr)funcArgs.getVAddr();
	RAddr raddr = pthread->virt2real(addr);
	*(long long*)raddr = globalClock;
	printf("global clock = %ld\n",globalClock);
	return pthread->getRetIcode();
}
OP(mint_sesc_main_clear)
{
	ThreadContext* main = pthread->getMainThreadContext();
	main->clear();
	return pthread->getRetIcode();
}
OP(mint_sesc_get_num_threads)
{
	MintFuncArgs funcArgs(pthread,picode);
	int32_t addr = (RAddr)funcArgs.getVAddr();
	RAddr raddr = pthread->virt2real(addr);
	*(int*)raddr = osSim->threadnum;
	return pthread->getRetIcode();
}
OP(mint_sesc_set_num_threads)
{
	MintFuncArgs funcArgs(pthread,picode);
	int i = funcArgs.getInt32();
	osSim->threadnum = i;
	osSim->recordThread = i;
	return pthread->getRetIcode();
}
...
{
	MintFuncArgs funcArgs(pthread,picode);
	RAddr addr = (RAddr)funcArgs.getVAddr();
	int32_t n = funcArgs.getInt32();
	/*for now it's only for int to increase*/
	RAddr raddr = pthread->virt2real(addr);
//	printf("INFO: raddr %x old = %d n = %d",raddr,*(int*)raddr,n);
	*(int*)raddr = *(int*)raddr + n;
//	printf("INFO: new = %d\n",*(int*)raddr);
	return pthread->getRetIcode();
}
OP(mint_sesc_version_remote){
	  MintFuncArgs funcArgs(pthread,picode);
	  RAddr addr = (RAddr)funcArgs.getVAddr();
	  int32_t version   = funcArgs.getInt32();
	  int32_t iteration = funcArgs.getInt32();
	  int32_t  size = funcArgs.getInt32();

	VTable::iterator it1 = pthread->vt.begin();
	for(;it1 != pthread->vt.end();it1++)
		if((*it1)->addr == addr)
			return pthread->getRetIcode();
	  
	ShareAddr::iterator it = pthread->addrs.begin();
	for(;it!=pthread->addrs.end();it++)
		if((*it) == addr)
			return pthread->getRetIcode();
	pthread->addrs.push_back(addr);

	if(pthread->findVTEntry(addr,size) == NULL){//添加版本号条目
			VersionTag *tmp = (VersionTag*)malloc(sizeof(VersionTag));
			tmp->addr = addr;
			tmp->size = size;
			tmp->preVersion = version;
			tmp->realAddr = addr;
			tmp->iteration = iteration;
			tmp->version = 0;
			pthread->vt.push_back(tmp);
//			printf("add remote data pid = %d,addr = %x  iteration %d request version %d\n",pthread->getPid(),addr,iteration,version);
	}
	return pthread->getRetIcode();
}
...
OP(mint_sesc_spawn){
  ...
  */
  ...
  if(flags)
  {
	  ...
	  flags = 0;
	  child->iteration = ThreadContext::nIterations;
	  ThreadContext::nIterations++;
  }
  else
  	child->setSpecThread(false);
  ...
  if( entry ) {
	...
	temp.entry = entry;
	temp.intArg1Reg  = arg;
	temp.intArg2Reg = Stack_size;
	temp.jmpPtrReg = entry;
	temp.retAddrReg = Exit_addr;
	temp.ppid = ppid;
	child->setThreadPara(temp);
//	printf("pid = %d, iteration = %d\n",child->getPid(),child->iteration);
#endif
  }else{
...
//#ifdef SESC_COW_REPLAY
#ifdef SESC_COW_UPDATE
	picode->specInst = pthread->isSpecThread();
	if(pthread->isSpecThread()){
	 	picode->exitInst = 1;
		
		 bool isInOrder = rsesc_is_order_commit(pthread->getPid());
		 if(!isInOrder){
		 	return picode;
		 }
		  ThreadContext::getContext(pthread->getPid())->copy(pthread);
		  bool ret;
		  if(pthread->getPid() != ThreadContext::getMainThreadContext()->getPid())
		  	ret = rsesc_commit(pthread);
		  ThreadContext::getContext(pthread->getPid())->copy(pthread);
			
		   if(ret) {
/*
			  	picode->nCommitPage = pthread->nCommitPage;
				picode->nCmpEntry = pthread->nCmpEntry;
*/
				rsesc_exit(pthread->getPid(),pthread->getIntArg1());
			   }//END OF IF(RET)
		   else{
			    	rsesc_noncommit_exit();
			 	rsesc_threads_replay();
	   		}//ENDOF ELSE
		}
	else{
//		printf("INFO:pid %d exit\n",pthread->getPid());
		rsesc_exit(pthread->getPid(),pthread->getIntArg1());
	}
#else
	rsesc_exit(pthread->getPid(),pthread->getIntArg1());
