...
{ 
...
#ifdef MEMLEVEL
	printf("MemorySystem:buildMemoryObj  section = %s,name = %s\n",device_descr_section,device_name);
#endif
  ...
  /*
  #ifdef SESC_COW
  else if(!strcasecmp(device_type,k_elm)){
   new_memory_device = new ELM(this,device_descr_section,device_name);
  }
  #endif
  */
  #ifdef SESC_COW_UPDATE
  else if(! strcasecmp(device_type,k_ecache)){
   new_memory_device = new ECache(this,device_descr_section,device_name);
  }
  #endif
  else {
