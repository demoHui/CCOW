...
bool rsesc_commit(ThreadContext * pthread)
{
	return osSim->eventCommit(pthread);
}

bool rsesc_is_order_commit(Pid_t pid)
{
	return osSim->isOrderCommit( pid);
}
void rsesc_noncommit_exit()
{
	osSim->eventNonCommitExit();
}
void rsesc_threads_replay()
{
	osSim->eventThreadsReplay();
}
#endif
int32_t rsesc_exit(int32_t cpid, int32_t err)
{
#ifdef SESC_COW_UPDATE
  OSSim::ExitThread* pthread = (OSSim::ExitThread*)malloc(sizeof(OSSim::ExitThread));
  pthread->pid = cpid;
  pthread->err = err;
  pthread->specThread = osSim->getContext(cpid)->isSpecThread();
  osSim->dts.push_back(pthread);
#else
